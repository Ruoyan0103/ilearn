...
def get_struct_volume(a = 3.90, c = 2.79):
    lattice = np.array([[a, 0, 0], [0, a, 0], [0.5*a, 0.5*a, 0.5*c]])
    basis   = np.array([[0, -0.25*a, 0.125*c], [0, 0.25*a, -0.125*c]])
    pbc = [1, 1, 1]
    symbols = 'Ge2'                                                     # two atoms in the primitive cell
    Ge_betaTin = Atoms(symbols, positions=basis, cell=lattice, pbc=pbc)
    n = len(Ge_betaTin)                                                 # number of atoms      # TOTEN/number of atoms
    v = Ge_betaTin.get_volume()/n                                       # volume/number of atoms
    return Ge_betaTin, v
...
def generate_data(folder, vfile, a_sat, a_end, interval):
    ...
    c_a_ratio=0.5527
    for a in a_list:
        Ge_betaTin, v = get_struct_volume(a, a*c_a_ratio)

        subfolder = os.path.join(folder, str(round(a, 3)))
        if not os.path.exists(subfolder): os.makedirs(subfolder)
        write(os.path.join(subfolder, 'bulk.data'), Ge_betaTin, format='lammps-data')
...
folder = 'Final2'
vfile = 'a-vol'
a_sat, a_end, interval = 4.6, 5.8, 0.1
