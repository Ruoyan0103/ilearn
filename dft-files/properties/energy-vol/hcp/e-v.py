...
def calLattice_constant(lattice_constant):
    atoms = bulk('Ge', 'hcp', a=lattice_constant, cubic=False)
...
### Save POSCAR
    ...
    write(folder + '/bulk' + '.data', atoms, format='lammps-data') 

    n = len(atoms)                                          
    v = atoms.get_volume()/n                                        # volume/number of atoms
    with open('Final/lattice-vol', 'a') as file:
        file.write(str(lattice_constant) + ' , ' + str(v) + '\n')
