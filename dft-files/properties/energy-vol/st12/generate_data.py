...
def get_struct_volume(a = 5.92, c = 7.03):
    x1 = 0.0874
    x2 = 0.1709
    y2 = 0.3704
    z2 = 0.2525
    ...
    pbc = [1, 1, 1]
    symbols = 'Ge12'                                                     # 12 atoms in the primitive cell
    Ge_st12 = Atoms(symbols, positions=basis, cell=lattice, pbc=pbc)

    n = len(Ge_st12)                                                 # number of atoms
    v = Ge_st12.get_volume()/n                                       # volume/number of atoms
    return Ge_st12, v
...
def generate_data(folder, vfile, a_sat, a_end, interval):
    ...
    c_a_ratio=1.182
    for a in a_list:
        Ge_st12, v = get_struct_volume(a, a*c_a_ratio)

        subfolder = os.path.join(folder, str(round(a, 3)))
        if not os.path.exists(subfolder): os.makedirs(subfolder)
        write(os.path.join(subfolder, 'bulk.data'), Ge_st12, format='lammps-data')
...
def sortFile(vfile, folder, engfile, outfile):
    natoms = 12                              # modify manualy, e.g. for beta/hcp/dia=2, f/bcc=1, bc8=8, hd=4, st12=12
    volume = []
...
folder = 'Final2'
vfile = 'a-vol'
a_sat, a_end, interval = 5.4, 6.7, 0.1
